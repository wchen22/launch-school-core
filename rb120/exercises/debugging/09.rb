...
class PostalService < CommunicationsProvider
  # This was the solution. Without including Mailing, when we call send on johns_postal_service, Ruby goes through the method lookup path for a `send` method and eventually finds one in the `Object` class further up on the ancestor chain.
