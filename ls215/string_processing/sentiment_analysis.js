...
function sentiment(text) {
  ...
  let wordFoundIn = (word, array) =>  array.indexOf(word) !== -1 ;
  let positivesMatched = words.filter(word => wordFoundIn(word, positiveWords));
  let negativesMatched = words.filter(word => wordFoundIn(word, negativeWords));

  let positiveCount = positivesMatched.length;
  let negativeCount = negativesMatched.length;

  console.log(`There are ${positiveCount} positive words in the text.\nPositive sentiments: ${positivesMatched.join(', ')}`)

  console.log(`There are ${negativeCount} negative words in the text.\nNegative sentiments: ${negativesMatched.join(', ')}`)

  let sentiment = positiveCount - negativeCount;
  if (sentiment > 0) {
    sentiment = 'Positive';
  } else if (sentiment < 0) {
    sentiment = 'Negative';
...
sentiment = function(text) {
  ...
  let positivesMatched = text.match(positiveRegex).map(word => word.toLowerCase());//words.filter(word => positiveRegex.test(word));
  let negativesMatched = text.match(negativeRegex).map(word => word.toLowerCase());
