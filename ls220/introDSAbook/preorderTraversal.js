...
function preorderTraversal(root) { // NLR
  // starting with current node
    // process the node
    // if a
  let output = [];
  const preorderTraversalWithArray = (root, array) => {
    array.push(root.val);

    if (!root.left && !root.right) return;
    if (root.left) preorderTraversalWithArray(root.left, array);
    if (root.right) preorderTraversalWithArray(root.right, array);

    return array;
  };
  
  return preorderTraversalWithArray(root, output);
}
// Test Cases:
